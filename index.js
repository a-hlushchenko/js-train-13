...
function manipulateUrl(url) {
  ...
  obj.protocol = "https";
  obj.host = "newhost.com";
  obj.searchParams.append("newParam", "newValue");
  obj.searchParams.delete("oldParam");

  return {
    href: obj.href,
    protocol: obj.protocol,
    host: obj.host,
    pathname: obj.hostname,
    search: obj.search,
...
function searchParamsURL(url) {
  const obj = new URL(url).searchParams;
  const map = new Map();
...
function createURLWithParams(params, url) {
  ...
  for (const key in params) {
    obj.searchParams.append(key, params[key]);
  }
...
function manipulateQuery(url, options) {
  ...
  console.log(options.get("delete"));
...
function getURLValues(url) {
  ...
  for (const key of newUrl.keys()) {
    arr.push(newUrl.getAll(key));
  }
